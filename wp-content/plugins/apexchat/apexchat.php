...
define( 'APEXCHAT_CURRENT_VERSION', '1.3.0' );
...
function apexchat_insert_function() {
	...
	if ( $options[ 'apexchat_setting_enabled' ] ) {
		if ( ( $options[ 'apexchat_admin_disbled' ] ) && ( !is_user_logged_in() ) ) {
			wp_enqueue_script( 'apexchat', "//www.apex.live/scripts/invitation.ashx?company=" . $options['apexchat_setting_companykey'], array(), null );
		}
