...
if(function_exists('su_add_shortcode')) {
	...
	su_add_shortcode(
		array(
			'id'       => 'youtube_advanced',
			'callback' => 'tmf_shortcode_youtube_advanced',
	...
	function tmf_shortcode_youtube_advanced( $atts = null, $content = null ) {
		...
		$response = TMF_Admin_Shortcodes::get_youtube_video_info($video_id);
		...
		if($data) {
			...
			$video_title = !empty($atts['title']) ? esc_attr( $atts['title'] ) : $title;
			$video_description = !empty($atts['description']) ? esc_attr($atts['description']) : $description;
		...
		return $video_object .'<div class="su-youtube su-u-responsive-media-' . $atts['responsive'] . su_get_css_class( $atts ) . '"><iframe width="' . $atts['width'] . '" height="' . $atts['height'] . '" src="https://' . $domain . '/embed/' . $video_id . '?' . $url_params . '" frameborder="0" allowfullscreen allow="autoplay; encrypted-media; picture-in-picture" title="' . esc_attr( $atts['title'] ) . '"></iframe></div>';
