...
class TMF_Shortcode {
	...
	public static function render_youtube ($options = array()) {
		...
		if (!empty($options['placeholder_src']))
			return '<img class="video-light-box youtube" src="' . $options['placeholder_src'] . '" data-video-url="//www.youtube.com/embed/'. $options['id'] .'?autoplay=1&rel=0" data-video-id="'. $options['id'] .'" data-video-ratio="'. $ratio .'" />';

		return '<div class="tmf-video youtube" style="max-width: '.$options['width'].'px;"><div data-responsive-padding="'. $ratio  .'%"><iframe src="//www.youtube.com/embed/'. $options['id'] .'?rel=0" frameborder="0" allowfullscreen></iframe></div></div>';
	}
