<div class="<?php echo $post->css_classes('medium') ?>">
	...
	<?php if($tmf_option->post_type_staff_read_more_buttons != 1): ?>
		<a href="<?php echo $post->permalink ?>" class="top read-more-button tmf-button small" title="View <?php echo $post->person_name ?>'s Staff Profile">
			View <?php echo $post->first_name ?>'s Profile
		</a>
	<?php endif; ?>

	<?php if ($post->has_thumbnail_image()): ?>
		<a href="<?php echo $post->permalink ?>" title="View <?php echo $post->person_name ?>'s Staff Profile">
	...
	<?php if($tmf_option->post_type_staff_read_more_buttons != 1): ?>
		<a href="<?php echo $post->permalink ?>" class="bottom read-more-button tmf-button small" title="View <?php echo $post->person_name ?>'s Staff Profile">
			View <?php echo $post->first_name ?>'s Profile
		</a>
	<?php endif ?>
