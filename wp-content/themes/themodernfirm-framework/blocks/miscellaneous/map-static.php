...
$apiKeys = 'AIzaSyDEjqVOz0DK4huA6DrUAseP0TbdMjbzgvE';
?>

<?php
global $tmf_option;
	
// If stored map is being used, render that
if($tmf_option->static_map_storage && isset($post)) {
	...
	$url = UPLOADS_URI . 'misc-images/'. $post->map_image;
}else{
	// Fetch map via google api otherwise
	$url = 'https://maps.googleapis.com/maps/api/staticmap?key='. $apiKeys .'&zoom='. $zoom .'&sensor=false&markers=color:red%%7C'. $address;
}
